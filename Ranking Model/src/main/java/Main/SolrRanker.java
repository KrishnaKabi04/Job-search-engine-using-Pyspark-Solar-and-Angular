...
public class SolrRanker {
    ...
    public SolrTweet getSolrTweet(SolrDocument solrDocument) {
        ...
        List<String> tweetMediaText = dataParser.getArrayList(solrDocument.getOrDefault("tweetMediaText", "").toString());
        List<String> tweetAttachedLinks = dataParser.getArrayList(solrDocument.getOrDefault("tweetAttachedLinks", "").toString());
        return new SolrTweet(tweetID, userID, tweetDateTime, tweetText, tweetFavoriteCount, tweetQuoteCount, tweetReplyCount, tweetRetweetCount, tweetHashtags, tweetUserMentions, tweetMediaURL, tweetMediaText, tweetAttachedLinks);
    }
    ...
    public List<Map<SolrTweet, SolrUser>> getRawQueryResult(String query) {
        ...
        try{
            ...
            tweetQuery.set("q", "tweetText:" + query + " OR tweetMediaText:" + query);
            tweetQuery.setRows(10);
    ...
    public Map<SolrUser, List<SolrTweet>> getUserQueryResult(String query) {
        ...
        try {
            SolrQuery userQuery = new SolrQuery();
            userQuery.set("q", "userName:" + query);
            QueryResponse userResponse = solrUserClient.query(userQuery);
    ...
    public static void main(String[] args) {
        ...
        while(userOption != 4) {
            ...
            switch (userOption) {
                case 1:
                    for (Map<SolrTweet, SolrUser> result : ranker.getRawQueryResult(query)) {
                        for ( Map.Entry<SolrTweet, SolrUser> entry : result.entrySet()) {
                            SolrTweet key = entry.getKey();
