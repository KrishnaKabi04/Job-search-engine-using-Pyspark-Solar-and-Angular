...
public class SolrIndexer {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (File file : Objects.requireNonNull(dataset.listFiles())) {
                ...
                while((nextLine = reader.readNext()) != null) {
                    try{
                        ...
                        List<String> tweetMediaText = new ArrayList<>();
                        if (!tweetMediaURL.isEmpty()) {
                            for(String imageURL : tweetMediaURL) {
                                if (!imageURL.isEmpty()) {
                                    imageURL = imageURL.replaceAll("'","");
                                    try {
                                        BufferedImage image = ImageIO.read(new URL(imageURL));
                                        tweetMediaText.add(instance.doOCR(image));
                                    }
                        ...
                        SolrTweet solrTweet = new SolrTweet(tweetID, userID, userDateTime, tweetText, tweetFavoriteCount, tweetQuoteCount, tweetReplyCount, tweetRetweetCount, tweetHashtags, tweetUserMentions, tweetMediaURL, tweetMediaText, tweetAttachedLinks );
                        indexWriter.addOrUpdateTweet(tweetID, solrTweet);

                        countDataRows += 1;
                        if (countDataRows % 10000 == 0)
                            System.out.println("Processed " + countDataRows + " Records");
                    }
                    catch (Exception ignored) {
            ...
            System.out.println("Dataset URL Errors: " + countURLErrors);
